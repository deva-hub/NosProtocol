defmodule NosProtocol.Portal do
  ...
  defmacro __using__(opts) do
    quote bind_quoted: [opts: opts] do
      ...
      @doc false
      def terminate(:cant_authenticate, state) do
        Socket.send(socket, NosLib.serialize(%ErrorMessage{reason: :cant_authenticate}))
        NosProtocol.Portal.__terminate__(reason, state)
      end

      def terminate(:outdated_client, state) do
        Socket.send(socket, NosLib.serialize(%ErrorMessage{reason: :outdated_client}))
        NosProtocol.Portal.__terminate__(reason, state)
      end

      def terminate(:corrupted_client, state) do
        Socket.send(socket, NosLib.serialize(%ErrorMessage{reason: :corrupted_client}))
        NosProtocol.Portal.__terminate__(reason, state)
      end
  ...
  def enter_loop(module, {ref, parent, socket, transport, opts}) do
    ...
    crypto = Keyword.fetch!(opts, :crypto, Crypto.Login)
    timeout = Keyword.fetch!(opts, :timeout, 300_000)
  ...
  def parse(module, socket, <<"NsTeST", packet::binary>>) do
    handle(module, socket, NosLib.deserialize(packet))
  end

  defp parse(module, socket, packet) do
    module.terminate(:cant_authenticate, socket)
  end

  def handle(module, socket, packet) do
    case {client_version?(packet.client_vsn),
      ...
      {true, true} ->
        case module.connect(packet, socket) do
          {:ok, reply, socket} ->
            Socket.send(socket, NosLib.serialize(reply))
            module.terminate(:normal, socket)

          {:error, reply} ->
            Socket.send(socket, NosLib.serialize(%ErrorMessage{reason: reason}))
            Socket.close(socket)
